...
const SearchPage = () => {
    ...
    const handleSearch = async(e)=>{
        e.preventDefault()
        try {
            const {data:{content}} = await axios.get(`/api/v1/search/${isActive}/${searchTerm}`)
            setResults(content)
        } catch (error) {
             ...
             <div className='container mx-auto px-4 py-8'>
                <div className='flex justify-center gap-3 mb-4'>
                    <button className={`py-2 px-4 rounded ${
							isActive === "movie" ? "bg-red-600" : "bg-gray-800"
						} hover:bg-red-700`}
                    ...
                    <button className={`py-2 px-4 rounded ${
							isActive === "tv" ? "bg-red-600" : "bg-gray-800"
						} hover:bg-red-700`}
                    ...
                    <button className={`py-2 px-4 rounded ${
							isActive === "person" ? "bg-red-600" : "bg-gray-800"
						} hover:bg-red-700`}
                ...
                <form className='flex gap-2 items-stretch mb-8 max-w-2xl mx-auto' onSubmit={handleSearch}>
                   <input
						...
						placeholder={"Search for a " + isActive}
