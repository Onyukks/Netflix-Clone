...
const History = () => {
    ...
    useEffect(() => {
		const getSearchHistory = async () => {
			try {
				const {data:{content}}= await axios.get(`/api/v1/search/history`);
    ...
    const handleDelete = async(id)=>{
        try {
         await axios.delete(`/api/v1/search/${id}/history`)
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3  gap-4">
                 {
                    searchHistory?.map(entry=>(
                        <div key={entry.id} className='bg-gray-800 p-4 rounded flex items-start'>
                           <img src={ORIGINAL_IMG_BASE_URL + entry.image} alt="History Image" 
